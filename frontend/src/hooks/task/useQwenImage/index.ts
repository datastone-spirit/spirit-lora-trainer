 ...
 * 怎么可能会有bug！！！
 */
import type { QwenImageTrainingInfoResult } from "@/api/monitor";
import { qwenImageTrainingInfo } from "@/api/monitor";
...
export interface InitData {
	...
	taskId: string;
	/** api返回的任务数据 */
	result: QwenImageTrainingInfoResult;
	/** 是否显示训练提示弹窗 */
...
class QwenImageLoraMonitor implements TaskImplementation {
	/** 定时器 */
	...
	/** 任务类型 */
	private readonly taskType: TaskType = "qwen-image";
	/** 任务名称 */
	private readonly taskName: string = "Qwen Image";
	/** 事件订阅 */
	...
	/** 更新当前任务信息 */
	private updateCurrentTaskInfo(result?: QwenImageTrainingInfoResult): void {
	...
	/** 处理查询成功 */
	private handleQuerySuccess(result: QwenImageTrainingInfoResult): void {
		// 更新数据
		this.updateCurrentTaskInfo(result);
	...
	private handleQueryFailure(error: any): void {
		...
		// log
		console.error("查询 Qwen Image 训练任务信息出错：", error);
	}
...
let qwenImageLoraMonitor: QwenImageLoraMonitor | null = null;

export function useQwenImageLora() {
	if (!qwenImageLoraMonitor) {
		qwenImageLoraMonitor = new QwenImageLoraMonitor();
	}

	return {
		qwenImageLoraMonitor
	};
}
