		...
		<el-descriptions :title="data.id" border label-width="200px" :column="2">
			...
			<template v-else>
				...
				<el-descriptions-item label="损失平均值">
					{{ data.detail.average_loss }}
				</el-descriptions-item>
				<el-descriptions-item label="当前损失">
					{{ data.detail.current_loss }}
				</el-descriptions-item>
...
import type { QwenImageTrainingInfoResult } from "@/api/monitor/types";
...
export interface QwenImageDetailProps {
	data: QwenImageTrainingInfoResult;
}

const props = defineProps<QwenImageDetailProps>();
...
const progress = computed(() => {
	...
	if (typeof props.data.detail === "string") return "";
	const current_epoch = props.data.detail.current ?? 0;
	const total_epoch = props.data.detail.total_optimization_steps ?? 0;
	return `${calculatePercentage(current_epoch, total_epoch)}%`;
});
